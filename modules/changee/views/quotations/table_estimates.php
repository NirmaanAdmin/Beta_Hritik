...
foreach ($rResult as $aRow) {
    ...
    if ( (has_permission('changee_quotations', '', 'edit') || is_admin()) && $aRow[db_prefix() . 'pur_estimates.status'] != 2) {
    ...
    $row[] = '<a href="' . admin_url('changee/view_pur_request/' . $aRow['pur_request']) . '" onclick="init_pur_estimate(' . $aRow['id'] . '); return false;">' . $aRow['pur_rq_code'] .'</a>' ;
    ...
    $row[] = changee_get_status_approve($aRow[db_prefix() . 'pur_estimates.status']);
