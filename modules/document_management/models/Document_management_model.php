	...
	public function get_item($id = '', $where = '', $select = '')
	{
		...
		if ($id != '') {
			...
			if ($where != '') {
				...
				// Check if the $where contains a parent_id of 275 or greater
				if (preg_match('/parent_id\s*=\s*(\d+)/', $where, $matches)) {
					$parent_id = (int) $matches[1];
					if ($parent_id >= 275) {
						// Include items with parent_id = 265 if parent_id is 275 or greater
						$this->db->or_where('parent_id', 265);
					}
				}
			}
