...
if ($toggle_filter == 1) {
    ...
    $aColumns = [
        'id',
        'CONCAT("PO-", id) AS goods_receipt_code',
        'vendor',
        'id AS buyer_id',
        'kind',
        'id AS pr_order_id',
    ...
    $sIndexColumn = 'id';
    $where[] = 'AND tblpur_orders.goods_id = 0';
}
...
if ($toggle_filter == 1) {
    foreach ($rResult as $aRow) {
        ...
        for ($i = 0; $i < count($aColumns); $i++) {
            ...
            if ($aColumns[$i] == 'vendor') {

                $_data = wh_get_vendor_company_name($aRow['vendor']);
            } elseif ($aColumns[$i] == 'approve_status') {
                $_data = get_status_approve($aRow['approve_status']);
            } elseif ($aColumns[$i] == 'id AS pr_order_id') {
                $get_pur_order_name = '';

                if (($aRow['id'] != '') && ($aRow['id'] != 0)) {
                    $get_pur_order_name .= '<a href="' . admin_url('purchase/purchase_order/' . $aRow['id']) . '" >' . get_pur_order_name($aRow['id']) . '</a>';
                }
