...
function initDataTable(
  ...
  if (lastSegment === 'table_order_tracker' || $("table.table-invoices").length > 0 || $("table.table-table_pur_invoices").length > 0 || $("table.table-table_pur_invoice_payments").length > 0) {
...
$(document).on('click', '.estimate-annexture-list a', function (e) {
  e.preventDefault();
  var annexureKey = $(this).attr('aria-controls');
  if(annexureKey == 'annexure7') {
    calculate_multilevel_total(annexureKey);
  } else {
        ...
        if (!taxes.hasOwnProperty(taxname)) {
          if (taxrate != 0) {
            ...
            tax_row =
              ...
              slugify(taxname) +
              '"></td></tr>';
            $(annexure_subtotal).after(tax_row);
            taxes[taxname] = calculated_tax;
          }
        } else {
          // Increment total from this tax
...
function add_estimate_item_to_table(data, itemid, merge_invoice, bill_expense) {
  ...
  var regex = /<br[^>]*>/gi;
  $.when(
    get_taxes_dropdown_template(tax_name, data.taxname),
    get_estimate_unit_dropdown(unit_id, data.unit_id),
    get_estimate_purchase_items(item_code, item_name)
  ).done(function (tax_dropdown, estimate_unit_dropdown, estimate_purchase_dropdown) {
    // order input
    table_row +=
      '<input type="hidden" class="order" name="newitems[' +
      item_key +
    ...
    table_row +=
      '<td class="budget_area"><input type="number" name="newitems[' +
      item_key +
      '][budget_area]" value="' +
      data.budget_area +
      '" class="form-control"></td>';
    ...
    table_row += '<td class="taxrate hide"></td>';
...
function get_estimate_item_preview_values(tab) {
  ...
  response.budget_area = $('#' + tab + ' input[name="budget_area"]').val();
  response.qty = $('#' + tab + ' input[name="quantity"]').val();
  response.taxname = $('#' + tab + ' select[name="taxname"]').selectpicker("val");
...
function clear_estimate_item_preview_values(tab) {
  ...
  previewArea.find("textarea").val(""); // includes cf
  previewArea.find('input[name="budget_area"]').val("");
  previewArea.find('input[name="quantity"]').val(1);
  previewArea.find('select[name="taxname"]').selectpicker("val", "");
  previewArea.find('input[name="rate"]').val("");
  previewArea.find('input[name="unit"]').val("");
...
$('a[href="#final_estimate"]').on('shown.bs.tab', function (e) {
  localStorage.setItem('activeTab', 'final_estimate');
});

$('a[href="#area_summary"]').on('shown.bs.tab', function (e) {
  localStorage.setItem('activeTab', 'area_summary');
  calculate_area_summary_total();
});
...
$('a[href="#area_working"]').on('shown.bs.tab', function (e) {
  localStorage.setItem('activeTab', 'area_working');
  calculate_area_working_total();
});
...
$('a[href="#budget_summary"]').on('shown.bs.tab', function (e) {
  localStorage.setItem('activeTab', 'budget_summary');
});

$('a[href="#project_timelines"]').on('shown.bs.tab', function (e) {
  localStorage.setItem('activeTab', 'project_timelines');
});

var activeTab = localStorage.getItem('activeTab');
if (activeTab) {
  $('a[href="#' + activeTab + '"]').tab('show');
  if(activeTab == 'area_summary') {
    calculate_area_summary_total();
  } else if(activeTab == 'area_working') {
    calculate_area_working_total();
  }
} else {
  $('a[href="#final_estimate"]').tab('show');
}
...
// Append the added items to the preview to the table as items
function add_multilevel_item_to_table(data, itemid, merge_invoice, bill_expense) {
  // If not custom data passed get from the preview
  var tab = $('.detailed-costing-tab.active').attr('id');
  var annexure_id = $('.detailed-costing-tab.active').attr('data-id');
  ...
  data =
    typeof data == "undefined" || data == "undefined"
      ? get_multilevel_item_preview_values(tab)
      : data;
  ...
  var int_amount = data.int_area * data.int_rate;

  var int_master_area = "newintitems[" + item_key + "][int_master_area]";
  var int_fun_area = "newintitems[" + item_key + "][int_fun_area]";
  $("body").append('<div class="dt-loader"></div>');
  var regex = /<br[^>]*>/gi;
  $.when(
    get_master_area_dropdown(int_master_area, data.int_master_area),
    get_functionality_area_dropdown(int_fun_area, data.int_fun_area)
  ).done(function (master_area_dropdown, functionality_area_dropdown) {
    // order input

    table_row +=
      '<input type="hidden" class="annexure" name="newintitems[' +
      item_key +
      '][annexure]" value=' + annexure_id + '>';
    ...
    table_row +=
      '<td class="int_area"><input type="number" onblur="calculate_multilevel_total();" onchange="calculate_multilevel_total();" name="newintitems[' +
      item_key +
      '][int_area]" value="' +
      data.int_area +
      '" class="form-control"></td>';
    ...
    table_row +=
      '<td class="int_rate"><input type="number" onblur="calculate_multilevel_total();" onchange="calculate_multilevel_total();" name="newintitems[' +
      item_key +
      '][int_rate]" value="' +
      data.int_rate +
    ...
    table_row +=
      '<td class="int_amount" align="right">' + format_money(int_amount, true) + '</td>';

    table_row +=
      '<td><textarea name="newintitems[' +
      item_key +
      '][int_remarks]" class="form-control item_remarks" rows="5">' +
      data.int_remarks.replace(regex, "\n") +
      "</textarea></td>";

    table_row +=
      '<td><a href="#" class="btn btn-danger pull-left" onclick="delete_multilevel_item(this,' +
      itemid +
      '); return false;"><i class="fa fa-trash"></i></a></td>';
    ...
    setTimeout(function () {
      calculate_multilevel_total(tab);
    }, 15);
    ...
    init_datepicker();
    init_color_pickers();
    clear_multilevel_item_preview_values(tab);
...
// Get the preview main values
function get_multilevel_item_preview_values(tab) {
  var response = {};
  response.int_master_area = $('#' + tab + ' select[name="int_master_area"]').val();
  response.int_fun_area = $('#' + tab + ' select[name="int_fun_area"]').val();
  response.int_area = $('#' + tab + ' input[name="int_area"]').val();
  response.int_rate = $('#' + tab + ' input[name="int_rate"]').val();
  response.int_remarks = $('#' + tab + ' textarea[name="int_remarks"]').val();
  return response;
}

function clear_multilevel_item_preview_values(tab) {
  var previewArea = $('#' + tab + '').find("tr").eq(1);
  previewArea.find('select[name="int_master_area"]').selectpicker("val", "");
  previewArea.find('select[name="int_fun_area"]').selectpicker("val", "");
  previewArea.find('input[name="int_area"]').val("");
  previewArea.find('input[name="int_rate"]').val("");
  previewArea.find('textarea[name="int_remarks"]').val("");
}

function delete_multilevel_item(row, itemid) {
  var annexure = $('.detailed-costing-tab.active').attr('id');
  $(row)
    .parents("tr")
    .addClass("animated fadeOut", function () {
      setTimeout(function () {
        $(row).parents("tr").remove();
        calculate_multilevel_total(annexure);
      }, 50);
    });

  // If is edit we need to add to input removed_items to track activity
  if (itemid && $('input[name="isedit"]').length > 0) {
    $("#removed-multilevel-items").append(hidden_input("removed_multilevel_items[]", itemid));
  }
}

function calculate_multilevel_total(annexure = '') {
  if (!annexure) {
    annexure = $('.detailed-costing-tab.active').attr('id');
  }
  var int_amount,
  ...
  rows = $('#' + annexure + ' .table.has-calculations tbody tr.item');
  $.each(rows, function () {
    var area = parseFloat($(this).find("td.int_area input").val()) || 0;
    var rate = parseFloat($(this).find("td.int_rate input").val()) || 0;
    int_amount = accounting.toFixed(
      area * rate,
      app.options.decimal_places
    );
    int_amount = parseFloat(int_amount);
    $(this).find("td.int_amount").html(format_money(int_amount, true));
    subtotal += int_amount;
  });
  total = subtotal;
  $(".annexure_subtotal").html(format_money(subtotal));
...
var tabSubKeyCounts = 0;
function getNextItemSubKey() {
  if (!tabSubKeyCounts) {
    tabSubKeyCounts = 0;
  }
  tabSubKeyCounts += 1;
  return tabSubKeyCounts;
}

function add_sub_multilevel_item_to_table(sub_tab, parent_id, data = 'undefined') {
  data = typeof data == "undefined" || data == "undefined" ? get_sub_multilevel_item_preview_values(sub_tab) : data;
  var table_row = "";
  var item_key = getNextItemSubKey();
  ...
  // Check if quantity is number
  if (isNaN(data.sub_qty)) {
    data.sub_qty = 1;
  }

  // Check if rate is number
  if (data.sub_rate === "" || isNaN(data.sub_rate)) {
    data.sub_rate = 0;
  }

  var sub_amount = data.sub_rate * data.sub_qty;

  var sub_unit_id = "newsubintitems[" + item_key + "][sub_unit_id]";
  var item_name = "newsubintitems[" + item_key + "][item_name]";
  var item_code = data.sub_item_name;
  $("body").append('<div class="dt-loader"></div>');
  var regex = /<br[^>]*>/gi;
  $.when(
    get_estimate_unit_dropdown(sub_unit_id, data.sub_unit_id),
    get_estimate_purchase_items(item_code, item_name)
  ).done(function (estimate_unit_dropdown, estimate_purchase_dropdown) {
    // order input

    table_row +=
      '<input type="hidden" class="parent_id" name="newsubintitems[' +
      item_key +
    ...
    if(item_code) {
      table_row += '<td class="sub_pur_item_name">' + estimate_purchase_dropdown[0] + '</td>';
    } else {
      table_row += '<td class="sub_pur_item_name"><select id="'+item_name+'" name="'+item_name+'" data-selected-id="" class="form-control selectpicker item-select" data-live-search="true" ><option value="">Type at least 3 letters...</option></select></td>';
    }

    table_row +=
      '<td><textarea name="newsubintitems[' +
      item_key +
      '][sub_long_description]" class="form-control item_long_description" rows="5">' +
      data.sub_long_description.replace(regex, "\n") +
      "</textarea></td>";

    table_row +=
      '<td class="sub_budget_area"><input type="number" name="newsubintitems[' +
      item_key +
      '][sub_budget_area]" value="' +
      data.sub_budget_area +
      '" class="form-control"></td>';

    table_row +=
      '<td class="sub_qty"><input type="number" min="0" onblur="calculate_sub_multilevel_total(\''+sub_tab+'\');" onchange="calculate_sub_multilevel_total(\''+sub_tab+'\');" data-quantity name="newsubintitems[' +
      item_key +
      '][sub_qty]" value="' +
      data.sub_qty +
      '" class="form-control">' + estimate_unit_dropdown[0] + '</td>';
    ...
    table_row +=
      '<td class="sub_rate"><input type="number" onblur="calculate_sub_multilevel_total(\''+sub_tab+'\');" onchange="calculate_sub_multilevel_total(\''+sub_tab+'\');" name="newsubintitems[' +
      item_key +
      '][sub_rate]" value="' +
      data.sub_rate +
      '" class="form-control"></td>';
    ...
    table_row +=
      '<td class="amount" align="right">' +
      format_money(sub_amount, true) +
      "</td>";

    table_row +=
      '<td><textarea name="newsubintitems[' +
      item_key +
      '][sub_remarks]" class="form-control item_remarks" rows="5">' +
      data.sub_remarks.replace(regex, "\n") +
      "</textarea></td>";

    table_row +=
      '<td><a href="#" class="btn btn-danger pull-left" onclick="delete_sub_multilevel_item(this,' +
      undefined +
      '); return false;"><i class="fa fa-trash"></i></a></td>';

    table_row += "</tr>";

    $('#' + sub_tab + ' table.items tbody').append(table_row);
    ...
    setTimeout(function () {
      calculate_sub_multilevel_total('' + sub_tab + '');
    }, 15);
    ...
    init_datepicker();
    init_color_pickers();
    clear_sub_multilevel_item_preview_values(sub_tab);
...
function get_sub_multilevel_item_preview_values(sub_tab) {
  var response = {};
  response.sub_item_name = $('#' + sub_tab + ' select[name="sub_item_name"]').val();
  response.item_code = $('#' + sub_tab + ' input[name="item_code"]').val();
  response.sub_long_description = $('#' + sub_tab + ' textarea[name="sub_long_description"]').val();
  response.sub_budget_area = $('#' + sub_tab + ' input[name="sub_budget_area"]').val();
  response.sub_qty = $('#' + sub_tab + ' input[name="sub_quantity"]').val();
  response.sub_rate = $('#' + sub_tab + ' input[name="sub_rate"]').val();
  response.sub_remarks = $('#' + sub_tab + ' textarea[name="sub_remarks"]').val();
  response.sub_unit_id = $('#' + sub_tab + ' select[name="sub_unit_id"]').selectpicker("val");
  return response;
}

function clear_sub_multilevel_item_preview_values(sub_tab) {
  var previewArea = $('#' + sub_tab + '').find("tr").eq(1);
  previewArea.find("textarea").val(""); // includes cf
  previewArea.find('input[name="sub_budget_area"]').val("");
  previewArea.find('input[name="sub_quantity"]').val(1);
  previewArea.find('input[name="sub_rate"]').val("");
  previewArea.find('select[name="sub_unit_id"]').selectpicker("val", "");
  previewArea.find('select[name="sub_item_name"]').selectpicker("val", "");
  previewArea.find('input[name="item_code"]').val("");
}

function close_modals_html_preview(sub_tab) {
  var missing_sub_pur_items = true;
  var sub_overall_area_id = $('#' + sub_tab + ' .sub_overall_budget_area_id').val().trim();
  $('#' + sub_tab + ' .sub_pur_item_name select.item-select').each(function() {
    var subitemVal = $(this).val().trim();
    if(subitemVal === '') {
      missing_sub_pur_items = false;
    }
  });
  if (missing_sub_pur_items == false) {
    alert_float('warning', 'Please enter item details in all rows');
  } else if(sub_overall_area_id === '') {
    alert_float('warning', 'Please enter overall area in this modal');
  } else {
    $('#'+sub_tab).modal('hide');
    var update_row = calculate_sub_multilevel_total(sub_tab, true);
    $('input[data-expand="' + sub_tab + '"]').val(update_row);
  }
}

function delete_sub_multilevel_item(row, itemid, sub_tab) {
  $(row)
    .parents("tr")
    .addClass("animated fadeOut", function () {
      setTimeout(function () {
  ...
  // If is edit we need to add to input removed_items to track activity
  if (itemid && $('input[name="isedit"]').length > 0) {
    $("#removed-sub-multilevel-items").append(hidden_input("removed_sub_multilevel_items[]", itemid));
  }
}

function uploadsubmultilevelitemcsv(sub_tab, parent_id) {
    "use strict";
    var fileInput = $('#' + sub_tab).find('#sub_file_csv')[0];
    var file = fileInput?.files[0];
    if (!file) {
      alert("Please select a file.");
      return;
    ...
    reader.onload = function (e) {
        ...
        if(totalRows > 0) {
          var cleanedData = jsonData.map(row => ({
            description: "",
            sub_long_description: row["Description"]?.trim() || "",
            sub_qty: row["Qty"]?.toString().replace(/,/g, "").trim() || "0",
            sub_rate: row["(*)Rate"]?.toString().replace(/,/g, "").trim() || "0",
            sub_remarks: row["Remarks"]?.trim() || "",
            taxname: [],
            sub_unit_id: '',
          }));
          if(cleanedData.length > 0) {
            insertSubMultilevelRowsSequentially(sub_tab, parent_id, cleanedData);
            fileInput.value = '';
          }
        }
    };
...
function insertSubMultilevelRowsSequentially(sub_tab, parent_id, data, delay = 50) {
  let index = 0;
  function next() {
    if (index < data.length) {
      add_sub_multilevel_item_to_table(sub_tab, parent_id, data[index]);
      index++;
      setTimeout(next, delay);
    }
  }
...
function calculate_sub_multilevel_total(sub_tab, update_main_row = false) {
  var int_amount,
  subtotal = 0,
  total = 0,
  quantity = 1,
  rows = $('#' + sub_tab + ' .table.has-calculations tbody tr.item'),
  sub_overall_budget_area = $('#' + sub_tab + ' .sub_overall_budget_area_id').val();
  sub_overall_budget_area = sub_overall_budget_area || 1;

  $.each(rows, function () {
    var qty = parseFloat($(this).find("td.sub_qty input").val()) || 0;
    var rate = parseFloat($(this).find("td.sub_rate input").val()) || 0;
    int_amount = accounting.toFixed(
      qty * rate,
      app.options.decimal_places
    );
    int_amount = parseFloat(int_amount);
    $(this).find("td.sub_amount").html(format_money(int_amount, true));
    subtotal += int_amount;
  });
  total = subtotal;
  subtotal = subtotal / sub_overall_budget_area;
  if(update_main_row == true) {
    return subtotal;
  }
  $('#' + sub_tab + ' .sub_multilevel_subtotal').html(format_money(subtotal));
  $('#' + sub_tab + ' .sub_multilevel_total').html(format_money(total));
  $(document).trigger("sales-total-calculated");
}
...
$("body").on("change", ".sub_overall_budget_area_id", function (e) {
  var sub_tab = $(this).data('oarea');
  calculate_sub_multilevel_total(sub_tab);
});
