...
$(function () {
  ...
  $("body").on("click", ".transaction-submit", function () {
    ...
    $('.pur_item_name select.item-select').each(function() {
      ...
      if(itemVal === '') {
        missing_pur_items.push('Please enter item details in ' + annexureName);
      }
    });
    var uniqueItems = [...new Set(missing_pur_items)];
    if (uniqueItems.length > 0) {
      alert_float('warning', uniqueItems.join('<br>'));
    } else {
      form.submit();
    }
  });
...
function add_sub_multilevel_item_to_table(sub_tab, parent_id, data = 'undefined') {
  ...
  $.when(
    ...
    if(item_code) {
      table_row += '<td>' + estimate_purchase_dropdown[0] + '</td>';
    } else {
      table_row += '<td class=""><select id="'+item_name+'" name="'+item_name+'" data-selected-id="" class="form-control selectpicker item-select" data-live-search="true" ><option value="">Type at least 3 letters...</option></select></td>';
    }
