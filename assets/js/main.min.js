...
$(document).on('click', '.estimate-annexture-list a', function (e) {
  ...
  if(annexureKey == 'annexure7') {
    calculate_interior_total(annexureKey);
  } else {
...
// Append the added items to the preview to the table as items
function add_interior_item_to_table(data, itemid, merge_invoice, bill_expense) {
  // If not custom data passed get from the preview
  var tab = $('.detailed-costing-tab.active').attr('id');
  var annexure_id = $('.detailed-costing-tab.active').attr('data-id');
  ...
  data =
    typeof data == "undefined" || data == "undefined"
      ? get_interior_item_preview_values(tab)
      : data;
  ...
  $.when(
    ...
    var interior_expand = '<br><button class="btn btn-info pull-left mright10 display-block" data-toggle="modal" data-target="#interiorExpand">Expand</button>';

    table_row += '<td>' + master_area_dropdown[0] + interior_expand + '</td>';

    table_row += '<td>' + functionality_area_dropdown[0] + '</td>';

    table_row +=
      '<td class="int_area"><input type="number" onblur="calculate_interior_total();" onchange="calculate_interior_total();" name="newintitems[' +
      item_key +
      '][int_area]" value="' +
      data.int_area +
      '" class="form-control"></td>';
    ...
    table_row +=
      '<td class="int_rate"><input type="number" onblur="calculate_interior_total();" onchange="calculate_interior_total();" name="newintitems[' +
      item_key +
      '][int_rate]" value="' +
      data.int_rate +
      '" class="form-control"></td>';
    ...
    table_row +=
      '<td><a href="#" class="btn btn-danger pull-left" onclick="delete_interior_item(this,' +
      itemid +
      '); return false;"><i class="fa fa-trash"></i></a></td>';
    ...
    init_datepicker();
    init_color_pickers();
    clear_interior_item_preview_values(tab);
...
// Get the preview main values
function get_interior_item_preview_values(tab) {
  var response = {};
...
function clear_interior_item_preview_values(tab) {
  var previewArea = $('#' + tab + '').find("tr").eq(1);
...
function delete_interior_item(row, itemid) {
  var annexure = $('.detailed-costing-tab.active').attr('id');
  $(row)
    .parents("tr")
    .addClass("animated fadeOut", function () {
      setTimeout(function () {
        $(row).parents("tr").remove();
        calculate_interior_total(annexure);
      }, 50);
    });

  // If is edit we need to add to input removed_items to track activity
  if (itemid && $('input[name="isedit"]').length > 0) {
    $("#removed-interior-items").append(hidden_input("removed_interior_items[]", itemid));
  }
}

function calculate_interior_total(annexure = '') {
  if (!annexure) {
    annexure = $('.detailed-costing-tab.active').attr('id');
  }
  var int_amount,
