...
/**
 * Order tracker attachments upload array
 * Multiple order tracker attachments can be upload if input type is array or dropzone plugin is used
 * @param  mixed $related related
 * @param  mixed $id id
 * @param  string $index_name attachments index, in different forms different index name is used
 * @return mixed
 */
function handle_order_tracker_attachments_array($related, $id, $index_name = 'attachments')
{
    if (!is_dir(get_upload_path_by_type('purchase'))) {
        mkdir(get_upload_path_by_type('purchase'), 0755);
    }
    if (!is_dir(get_upload_path_by_type('purchase') . 'pur_order_tracker')) {
        mkdir(get_upload_path_by_type('purchase') . 'pur_order_tracker', 0755);
    }
    if (!is_dir(get_upload_path_by_type('purchase') . 'pur_order_tracker/'. $related)) {
        mkdir(get_upload_path_by_type('purchase') . 'pur_order_tracker/'. $related, 0755);
    }
    $uploaded_files = [];
    $path           = get_upload_path_by_type('purchase') . 'pur_order_tracker/' . $related . '/' . $id . '/';
    $CI             = &get_instance();
